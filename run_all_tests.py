...
with open('vcore_kernel.cu', 'r') as f:
    cuda_code = f.read()
    # Отключаем стандартный инклюд, если он конфликтует с компилятором CuPy NVRTC
    ...
    # Запуск ядра для инициализации контекста
    ...
    # TEST 3: VRAM Memory Leakage (ИСПРАВЛЕНО: переведено с PyTorch на CuPy Memory Pool)
